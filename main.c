...
int main(void) {
  alignas(64) uint64_t value =
      123; // Align value to a typical 64-byte cache-line boundary.
  volatile uint64_t
      result; // Prevent the compiler from discarding the measured load.

  result = value; // Warm value's cache line so the next access should be
                  // cache-resident.
                  //
  uint64_t start = start_timer();
  uint64_t overhead = stop_timer() - start;

  // Measure an access to the warmed cache line, which will likely be served by
  // L1.
  start = start_timer();
  result = value;
  ...
  _mm_clflush(
      (void *)&value); // Evict value's cache line from the processor's caches.
